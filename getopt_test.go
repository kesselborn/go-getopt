...
func TestEnvironmentValueParsing(t *testing.T) {
	...
	if opts, _, _, _ := options.ParseCommandLine("", 0); !equalIntArray(opts["ports"].IntArray, []int64{4000, 4001, 4002}) {
		t.Errorf("did not recognize option set via ENV variable (PORTS=4000,4001,4002)")
	}

	os.Args = []string{"prog"}
	os.Setenv("KEYS", "faa,bor,boz")
	if opts, _, _, _ := options.ParseCommandLine("", 0); !equalStringArray(opts["keys"].StrArray, []string{"faa", "bor", "boz"}) {
		t.Errorf("did not recognize option set via ENV variable (KEYS=faa,bor,boz)")
	}

	os.Args = []string{"prog"}
	os.Setenv("LOGFILE", "/tmp/logfile")
	if opts, _, _, _ := options.ParseCommandLine("", 0); opts["logfile"].String != "/tmp/logfile" {
...
func TestDefaultValueParsing(t *testing.T) {
	...
	if opts, _, _, _ := options.ParseCommandLine("", 0); opts["instances"].Int != 4 {
		t.Errorf("did not recognize default value (int instances)")
	}

	if opts, _, _, _ := options.ParseCommandLine("", 0); !equalStringArray(opts["keys"].StrArray, []string{"foo", "bar", "baz"}) {
		t.Errorf("did not recognize default value (string array keys)")
	}

	if opts, _, _, _ := options.ParseCommandLine("", 0); opts["logfile"].String != "/var/log/foo.log" {
	...
	if opts, _, _, _ := options.ParseCommandLine("", 0); !equalIntArray(opts["ports"].IntArray, []int64{3000, 3001, 3002}) {
		t.Errorf("did not recognize default value (int64 array ports)")
	}

	if opts, _, _, _ := options.ParseCommandLine("", 0); !equalIntArray(opts["secondaryports"].IntArray, []int64{5001, 5002, 5003}) {
...
func TestArgumentsParsing(t *testing.T) {
	...
	if opts, arguments, _, _ := options.ParseCommandLine("", 0); !equalStringArray(arguments, []string{"foobar", "barbaz"}) || opts["debug"].Bool != true {
	...
	if opts, arguments, _, _ := options.ParseCommandLine("", 0); !equalStringArray(arguments, []string{"foobar", "barbaz"}) ||
	...
	if opts, arguments, _, _ := options.ParseCommandLine("", 0); !equalStringArray(arguments, []string{"barbaz", "foobar"}) ||
		opts["debug"].Bool != true ||
		!equalIntArray(opts["ports"].IntArray, []int64{5000, 6000}) {
