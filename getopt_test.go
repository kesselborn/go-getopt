...
func TestShortOptionsFlagsParsing(t *testing.T) {
  ...
  if opts, _, _, _ := options.parse([]string{"-d"}, []string{}, "", 0 );
  ...
  if opts, _, _, _ := options.parse([]string{"-d", "-v"}, []string{}, "", 0 );
...
func TestShortOptionRequiredParsing(t *testing.T) {
  ...
  if opts, _, _, _ := options.parse([]string{"-m", "heartbeat"}, []string{}, "", 0 );
    opts["method"].String != "heartbeat" {
      t.Errorf("method optioned wasn't parsed correctly expected 'heartbeat', was '" + opts["method"].String + "'")
  }

  if opts, _, _, _ := options.parse([]string{"-mheartbeat"}, []string{}, "", 0 );
    opts["method"].String != "heartbeat" {
      t.Errorf("method optioned wasn't parsed correctly expected 'heartbeat', was '" + opts["method"].String + "'")
  }

  if _, _, _, err := options.parse([]string{"-m"}, []string{}, "", 0 );
    err == nil || err.errorCode != MissingValue {
      t.Errorf("required option without value did not raise error")
  }

  if _, _, _, err := options.parse([]string{"-m", "-x"}, []string{}, "", 0 );
    err == nil || err.errorCode != MissingValue {
      t.Errorf("required option without value did not raise error")
  }

  if _, _, _, err := options.parse([]string{""}, []string{}, "", 0 );
...
func TestConcatenatedOptionsParsingSimple(t *testing.T) {
  ...
  if opts, _, _, _ := options.parse([]string{"-dv"}, []string{}, "", 0 );
    opts["debug"].Bool != true || opts["verbose"].Bool != true {
      t.Errorf("did not recognize all flags when concatenation options (2 options)")
  }

  if opts, _, _, _ := options.parse([]string{"-dvD"}, []string{}, "", 0 );
  ...
  if _, _, _, err := options.parse([]string{"-dvD"}, []string{}, "", 0 );
    err == nil || err.errorCode != MissingOption {
      t.Errorf("did not recognize a missing required option in concatenation mode")
  }

  if _, _, _, err := options.parse([]string{"-Dl"}, []string{}, "", 0 );
    err == nil || err.errorCode != MissingValue {
      t.Errorf("did not realize that I missed a value")
  }

  if _, _, _, err := options.parse([]string{"-Dl", "-d"}, []string{}, "", 0 );
    err == nil || err.errorCode != MissingValue {
      t.Errorf("did not realize that I missed a value")
  }
...
func TestConcatenatedOptionsParsingWithStringValueOptionAtTheEnd(t *testing.T) {
  ...
  if opts, _, _, _ := options.parse([]string{"-dvDl/tmp/log.txt"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
  ...
  if opts, _, _, _ := options.parse([]string{"-dvDmdaemon"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
  ...
  if opts, _, _, _ := options.parse([]string{"-dvDm", "daemon"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
    opts["dryrun"].Bool  != true || opts["mode"].String    != "daemon" {
...
func TestConcatenatedOptionsParsingWithIntValueOptionAtTheEnd(t *testing.T) {
  ...
  if opts, _, _, _ := options.parse([]string{"-dvDp3000"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
  ...
  if opts, _, _, _ := options.parse([]string{"-dvDp", "3000"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
    opts["dryrun"].Bool  != true || opts["port"].Int != 3000 {
      fmt.Printf("%#v", opts)
  ...
  if opts, _, _, _ := options.parse([]string{"-dvDi4"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
    opts["dryrun"].Bool  != true || opts["instances"].Int    != 4 {
      t.Errorf("did not recognize all flags when concatenation options (3 flags + 1 required)")
  }

  if opts, _, _, _ := options.parse([]string{"-dvDi", "4"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
    opts["dryrun"].Bool  != true || opts["instances"].Int    != 4 {
...
func TestConcatenatedOptionsParsingWithIntArrayValueOptionAtTheEnd(t *testing.T) {
  ...
  if opts, _, _, _ := options.parse([]string{"-dvDp5000,5001,5002"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
  ...
  if opts, _, _, _ := options.parse([]string{"-dvDp", "5000,5001,5002"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
    opts["dryrun"].Bool  != true || !equalIntArray(opts["ports"].IntArray, []int64{5000, 5001, 5002}) {
      t.Errorf("did not recognize all flags when concatenation options (3 flags + 1 optional int array separated by space)")
  }

  if opts, _, _, _ := options.parse([]string{"-dvDt10,20,30"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
  ...
  if opts, _, _, _ := options.parse([]string{"-dvDt", "10,20,30"}, []string{}, "", 0 );
    opts["debug"].Bool   != true || opts["verbose"].Bool != true ||
    opts["dryrun"].Bool  != true || !equalIntArray(opts["timeouts"].IntArray, []int64{10,20,30}) {
...
func TestEnvironmentValueParsing(t *testing.T) {
  ...
  if opts, _, _, _ := options.parse([]string{}, []string{"DEBUG=1"}, "", 0);
    opts["debug"].Bool != true {
      t.Errorf("did not recognize option set via ENV variable (DEBUG=1)")
  }
  if opts, _, _, _ := options.parse([]string{}, []string{"DEBUG=TRUE"}, "", 0);
    opts["debug"].Bool != true {
  ...
  if opts, _, _, _ := options.parse([]string{}, []string{"PORTS=4000,4001,4002"}, "", 0);
    !equalIntArray(opts["ports"].IntArray, []int64{4000,4001,4002}) {
      t.Errorf("did not recognize option set via ENV variable (PORTS=4000,4001,4002)")
  }

  if opts, _, _, _ := options.parse([]string{}, []string{"KEYS=faa,bor,boz"}, "", 0);
    !equalStringArray(opts["keys"].StrArray, []string{"faa","bor","boz"}) {
      t.Errorf("did not recognize option set via ENV variable (KEYS=faa,bor,boz)")
  }

  if opts, _, _, _ := options.parse([]string{}, []string{"LOGFILE=/tmp/logfile"}, "", 0);
    opts["logfile"].String != "/tmp/logfile" {
